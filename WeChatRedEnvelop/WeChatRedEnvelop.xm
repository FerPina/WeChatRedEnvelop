...
- (void)addHelperSection {
    id image = [[%c(MMThemeManager) sharedThemeManager] imageNamed:@"icons_outlined_colorful_favorites"];
    id cell = [%c(WCTableViewCellManager) normalCellForSel:@selector(goNewSetting) target:self leftImage:image title:@"微信小助手" badge:nil WithDisclosureIndicator:YES];
    
    WCTableViewSectionManager *section = [%c(WCTableViewSectionManager) defaultSection];
